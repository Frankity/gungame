...
local weapons = {
    m9 = {
        guid = Guid("B145A444-BC4D-48BF-806A-0CEFA0EC231B", "D"),
        attachments = {}
    }, -- https://github.com/Powback/Venice-EBX/blob/071473993867cd2297dc662517c61edaff51e8fe/Weapons/M9/U_M9.txt
    m44 = {
        guid = Guid("1EA227D8-2EB5-A63B-52FF-BBA9CFE34AD8", "D"),
        attachments = {}
    },
    g18 = {
        guid = Guid("DB364A96-08FB-4C6E-856B-BD9749AE0A92", "D"),
        attachments = {}
    },
    m1911 = {
        guid = Guid("A76BB99E-ABFE-48E9-9972-5D87E5365DAB", "D"),
        attachments = {}
    },
    spas12 = {
        guid = Guid("6D99F118-04BD-449A-BA0E-1978DDF5894D", "D"),
        attachments = {}
    },
    l96 = {
        guid = Guid("CBAEC77C-A6AD-4D63-96BD-61FCA6C18417", "D"),
        attachments = {unlocksForWeapons.l96pso}
    },
    ak74 = {
        guid = Guid("3BA55147-6619-4697-8E2B-AC6B1D183C0E", "D"),
        attachments = {unlocksForWeapons.ak74eotech}
    },
    p90 = {
        guid = Guid("C12E6868-FC08-4E25-8AD0-1C51201EA69B", "D"),
        attachments = {unlocksForWeapons.p90eotech}
    },
    l86 = {
        guid = Guid("BA0AF247-2E5B-4574-8F89-515DFA1C767D", "D"),
        attachments = {unlocksForWeapons.l86eotech}
    },
    m98 = {
        guid = Guid("05EB2892-8B51-488E-8956-4350C3D2BA27", "D"),
        attachments = {unlocksForWeapons.m96pso}
    },
    dao12 = {
        guid = Guid("27F63AEA-DD70-4929-9B08-5FF8F075B75E", "D"),
        attachments = {unlocksForWeapons.daoeotech}
    },
    mp7 = {
        guid = Guid("04C8604E-37DE-4B51-B70A-66468003D604", "D"),
        attachments = {unlocksForWeapons.mp7eotech}
    },
    ump45 = {
        guid = Guid("2A267103-14F2-4255-B0D4-819139A4E202", "D"),
        attachments = {unlocksForWeapons.ump45eotech}
    },
    m249 = {
        guid = Guid("AEAA518B-9253-40C2-AA18-A11F8F2D474C", "D"),
        attachments = {unlocksForWeapons.m249eotech}
    },
    type95 = {
        guid = Guid("FE05ACAA-32FC-4FD7-A34B-61413F6F7B1A", "D"),
        attachments = {unlocksForWeapons.type95eotech}
    },
    mg36 = {
        guid = Guid("95E00B23-BAD4-4F3B-A85E-990204EFF26B", "D"),
        attachments = {unlocksForWeapons.mg36eotech}
    },
    pp19 = {
        guid = Guid("CECC74B7-403F-4BA1-8ECD-4A59FB5379BD", "D"),
        attachments = {unlocksForWeapons.pp19eotech}
    }
}
...
local weaponOrder = {
    weapons.m9, weapons.m44, weapons.g18, weapons.m1911, weapons.spas12,
    weapons.l96, weapons.ak74, weapons.p90, weapons.l86, weapons.m98,
    weapons.dao12, weapons.mp7, weapons.ump45, weapons.m249, weapons.type95,
    weapons.mg36, weapons.pp19
}
...
function GunGameServer:OnPlayerJoining(playerName, PlayerGUID, ip) end

function GunGameServer:OnEngineUpdate(deltaTime, simDeltaTime)
    local players = PlayerManager:GetPlayers()
end

function GunGameServer:OnServerRoundOver(roundTime, winningTeam) self:ResetVars() end
...
function OnLevelLoaded(levelName, gameMode, round, roundsPerMap)
...
function GunGameServer:OnPlayerKilled(player, inflictor, position, weapon,
                                      roadkill, headshot, victimInReviveState)
    ...
    if playersScores[inflictor.id].score == 16 then
        ChatManager:SendMessage(inflictor.name ..
                                    " won the match, next round in 10 seconds..")
...
function GunGameServer:RegisterVars() self:ResetVars() end
...
function GunGameServer:OnExtensionUnloading() self:ResetVars() end
    ...
    for _, instance in pairs(instances) do
        ...
        if instance.typeInfo.name == 'UnlockAsset' then
            for i, sAsset in pairs(soldierAppearance) do
                local appearanceInatance =
                    ResourceManager:SearchForInstanceByGuid(sAsset)
...
function getRandomSpawnPoint() return spawnPlaces[math.random(#spawnPlaces)] end
...
function GunGameServer:UpdateWeapon(player)
    ...
    if playerScore ~= nil then score = playerScore.score end

    local weapon = ResourceManager:SearchForInstanceByGuid(
                       weaponOrder[score].guid)
    local attachments = {}

    for _, attGuid in pairs(weaponOrder[score].attachments) do
        local attachment = ResourceManager:SearchForInstanceByGuid(attGuid)
        if attachment ~= nil then table.insert(attachments, attachment) end
    end
...
function GunGameServer:OnRequestSpawn(player)
    if not player.alive then
        ...
        local transform = LinearTransform(Vec3(1, 0, 0), Vec3(0, 1, 0),
        ...
        print(transform.trans.x .. " " .. transform.trans.y .. " " ..
                  transform.trans.z)
        ...
        if soldier == nil then print("failed to create soldier") end

        player:SpawnSoldierAt(soldier, transform,
...
function GunGameServer:OnPlayerSpawn(player) end

function GunGameServer:ReadInstance(p_Instance, p_PartitionGuid, p_Guid) end

function GunGameServer:OnReceive(player)
    -- sending player scores table to the client which requested it
    local players = PlayerManager:GetPlayers()
    for _, p in pairs(players) do
        if playersScores ~= nil then playersScores[p.id].ping = p.ping end
    end
